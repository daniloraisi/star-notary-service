...
it("can add the star name and star symbol properly", async () => {
  let instance = await StarNotary.deployed();
  let starId = 6;
  await instance.createStar("awesome star", "AS", starId, {
    from: accounts[0],
  });
  assert.equal(
    (await instance.tokenIdToStarInfo.call(starId)).name,
    "awesome star"
  );
  assert.equal((await instance.tokenIdToStarInfo.call(starId)).symbol, "AS");
});
